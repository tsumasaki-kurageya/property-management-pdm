...
プロセスノードは正本の`order`順に並べ、候補を省略しない。

- PC・タブレットでは表示幅に応じて複数列へ折り返す
- スマートフォンでは1列にする
- 一覧には最大高を設け、収まらない候補は一覧内の縦スクロールで確認する
- プロセスID、プロセス名、`表示中`ラベルは省略しない
- 開始契機・完了状態はノード内で折り返す

これにより、最大12件の候補がある場合も中央の工程図を押し下げ続けず、全候補へ到達できるようにする。
