...
test('領域をホバーすると全所属業務を周辺へ展開し、明瞭な線で結ぶ', async ({ page }, testInfo) => {
  ...
  await expect(explorer.locator('.react-flow__edge')).toHaveCount(13);
  await expect(explorer.getByLabel(/業務 BM-10-01 /)).toBeVisible();

  const edgeStrokeWidth = await explorer.locator('.react-flow__edge-path').first().evaluate(
    (element) => getComputedStyle(element).getPropertyValue('stroke-width'),
  );
  expect(Number.parseFloat(edgeStrokeWidth)).toBeGreaterThanOrEqual(2);
});
